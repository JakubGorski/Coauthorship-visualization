...
app.layout = html.Div(style={'backgroundColor': colors['background']}, children=[
    ...
    html.Div(dcc.Dropdown(id='units-dropdown',
                          options=[
                              {'label': 'Whole WMiI', 'value': '1-wydzial-matematyki-i-informatyki-uj'},
                              {'label': 'Instytut informatyki i matematyki komputerowej', 'value': '2-instytut_informatyki_i_matematyki_komputerowej'},
                              {'label': 'Instytut Matematyki', 'value': '3-instytut_matematyki'},
                              {'label': 'Katedra Teorii Optymalizacji i Sterowania', 'value': '23-katedra_teorii_optymalizacji_i_sterowania'},
                              {'label': 'Zespół Katedr i Zakładów Informatyki Matematycznej', 'value': '4-zespol_katedr_i_zakladow_informatyki_matematycznej'},
                              {'label': 'Katedra Informatyki Stosowanej', 'value': '7-katedra_informatyki_stosowanej'},
                              {'label': 'Katedra Matematyki Obliczeniowej', 'value': '8-katedra_matematyki_obliczeniowej'},
                              {'label': 'Katedra Metod Efektywnych Algebry', 'value': '11-katedra_metod_efektywnych_algebry'},
                              {'label': 'Katedra Uczenia Maszynowego', 'value': '22-katedra_uczenia_maszynowego'},
                              {'label': 'Zakład Matematyki Dyskretnej', 'value': '10-zaklad_matematyki_dyskretnej'},
                              {'label': 'Katedra Analizy Funkcjonalnej', 'value': '12-katedra_analizy_funkcjonalnej'},
                              {'label': 'Katedra Analizy Matematycznej', 'value': '13-katedra_analizy_matematycznej'},
                              {'label': 'Katedra Funkcji Rzeczywistych', 'value': '14-katedra_funkcji_rzeczywistych'},
                              {'label': 'Katedra Geometrii', 'value': '16-katedra_geometrii'},
                              {'label': 'Katedra Geometrii Algebraicznej i Teorii Liczb', 'value': '24-katedra_geometrii_algebraicznej_i_teorii_liczb'},
                              {'label': 'Katedra Geometrii Analitycznej', 'value': '15-katedra_geometrii_analitycznej'},
                              {'label': 'Katedra Matematyki Stosowanej', 'value': '17-katedra_matematyki_stosowanej'},
                              {'label': 'Katedra Równań Różniczkowych', 'value': '18-katedra_rownan_rozniczkowych'},
                              {'label': 'Katedra Teorii Aproksymacji', 'value': '19-katedra_teorii_aproksymacji'},
                              {'label': 'Zakład Historii Matematyki', 'value': '20-zaklad_historii_matematyki'},
                              {'label': 'Zakład Matematyki Finansowej', 'value': '21-zaklad_matematyki_finansowej'},
                              {'label': 'Katedra Algorytmiki', 'value': '5-katedra_algorytmiki'},
                              {'label': 'Katedra Podstaw Informatyki', 'value': '6-katedra_podstaw_informatyki'}
                          ],
    ...
    html.Div(dcc.Graph(id='network-graph'),
             style={"height": "100%", "width": "55%", 'display': 'inline-block', 'padding': '0px 0px 20px 0px'}),
...
def save_final_data(jsonified_cleaned_list):
    ...
    df = data_to_df_group(cleaned_data)
...
def show_network(jsonified_cleaned_data, years, authors, jsonified_cleaned_list):
    ...
    f_df = df.loc[df['year'].isin(range(start_date, end_date))]
    if authors:
...
@app.callback(Output('publications-with-graph', 'figure'),
              ...
              [State('intermediate-value0', 'children'),
               State('intermediate-value', 'children')])
def show_publications_graph(clickData, jsonified_cleaned_data, jsonified_cleaned_list):
...
@app.callback(Output('publications-years-graph', 'figure'),
              [Input('network-graph', 'clickData'),
               Input('year-range-slider', 'value')],
              [State('intermediate-value0', 'children')])
def show_coauthors_graph(clickData, years, jsonified_cleaned_data):
    ...
    if text != name:
        ...
        x, y = [], []
        for val, cnt in filtered_df.author.value_counts().iteritems():
            if val != name:
                x.append(val)
                y.append(cnt)
        for val, cnt in filtered_df.coauthor.value_counts().iteritems():
            if val != name:
                x.append(val)
                y.append(cnt)
